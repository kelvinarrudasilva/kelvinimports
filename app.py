# app.py - NOVE STORE Dashboard com aba COMPRAS e busca avançada
...
def preparar_tabela_vendas(df, estoque_df=None):
    ...
    d["MEDIA CUSTO UNITARIO"] = d["MEDIA CUSTO UNITARIO"].map(
    ...
    try:
        d["VALOR VENDA_RAW"] = parse_money_series(df["VALOR VENDA"]).fillna(
            0
        )
    except Exception:
    ...
    try:
        d["CUSTO_RAW"] = parse_money_series(
            df["MEDIA CUSTO UNITARIO"]
        ).fillna(0)
    except Exception:
...
.kpi h3 { font-size: 17px; font-weight: 800; margin: 0 0 6px 0; }
.kpi .value { font-size: 26px; font-weight: 900; letter-spacing: -0.5px; }
</style>
"""
st.markdown(GLOBAL_CSS, unsafe_allow_html=True)

# ---------- TOPO COM LOGO NOVE STORE + BOTÃO ATUALIZAR ----------
...
with top_col_logo:
    try:
        ...
        st.markdown(
            "<div class='logo-wrap'><span style='font-weight:800;font-size:18px;'>NS</span></div>",
...
if "ESTOQUE" in dfs:
    ...
    for alt in [
        ...
        if alt in df_e.columns:
            df_e["Media C. UNITARIO"] = parse_money_series(df_e[alt]).fillna(
                0
            )
            break
    for alt in [
        ...
        if alt in df_e.columns:
            df_e["Valor Venda Sugerido"] = parse_money_series(
                df_e[alt]
            ).fillna(0)
            break
...
if "VENDAS" in dfs:
    ...
    if "DATA" in df_v.columns:
        df_v = df_v.sort_values("DATA", ascending=False).reset_index(
            drop=True
        )
    dfs["VENDAS"] = df_v
...
if "COMPRAS" in dfs:
    ...
    if ccols:
        df_c["CUSTO UNITÁRIO"] = parse_money_series(df_c[ccols[0]]).fillna(
            0
        )
...
with col_kpis:
    mostrar_saldos = st.toggle(
        "👁‍🗨 Mostrar faturamento e lucro",
        value=False,
        help="Por segurança, esses valores ficam ocultos por padrão.",
    )
    if mostrar_saldos:
        ...
        try:
            ...
            if not estoque_all.empty:
                last_sale = (
                    ...
                    .rename(columns={"DATA": "ULT_VENDA"})
                    if not vendas_all.empty
                    else pd.DataFrame(columns=["PRODUTO", "ULT_VENDA"])
                )
                last_buy = (
                    ...
                    .rename(columns={"DATA": "ULT_COMPRA"})
                    if not compras_all.empty
                    else pd.DataFrame(columns=["PRODUTO", "ULT_COMPRA"])
                )
                enc = (
                    estoque_all.merge(
...
with tabs[2]:
    ...
    if df_c.empty:
        ...
        if "DATA" in df_c.columns:
            df_c["DATA"] = pd.to_datetime(
                df_c["DATA"], errors="coerce"
            )
        ...
        for c in df_c.columns:
            if any(
                k in str(c).upper() for k in ("FORNEC", "SUPPLIER", "VENDOR")
            ):
                fornecedor_col = c
                break
        ...
        st.markdown(
            "### 📈 Visão rápida das compras (gráfico por produto/fornecedor)"
        )
        ...
        k1.metric(
            "Total compras (mês)",
            formatar_reais_sem_centavos(total_comp_mes),
        )
        k2.metric("Nº de compras", f"{n_comp_mes}")
        k3.metric(
            "Gasto marketing",
            formatar_reais_sem_centavos(marketing_total),
        )
        ...
        if marketing_total > 0:
            ...
            insights.append(
                "Nenhuma compra claramente marcada como 'marketing' encontrada — padronize a descrição para identificar gastos de anúncio/divulgação."
            )
        if not pivot.empty:
        ...
        insights.append(
            "Sugestão: use uma palavra-chave padrão (ex: 'ADS META' ou 'MARKETING') em OBS para facilitar análise de retorno."
        )
        for ins in insights:
            st.write("- " + ins)
...
with tabs[3]:
    st.markdown(
        """
    <div style='background: rgba(255,255,255,0.03); border-radius:14px; padding:10px; margin-bottom:8px;'>
    """,
        unsafe_allow_html=True,
    )
    ...
    for _, r in df_page.iterrows():
        nome = r.get("PRODUTO", "")
        estoque = (
            int(r.get("EM ESTOQUE", 0))
            if pd.notna(r.get("EM ESTOQUE", 0))
            else 0
        )
        venda = r.get("VENDA_FMT", "R$ 0")
        custo = r.get("CUSTO_FMT", "R$ 0")
        vendidos = (
            int(r.get("TOTAL_QTD", 0))
            if pd.notna(r.get("TOTAL_QTD", 0))
            else 0
        )
        iniciais = (
            "".join([p[0].upper() for p in str(nome).split()[:2] if p]) or "—"
        )
        badges = []
        if estoque <= 3:
