# app.py — Corrigido e otimizado
# Gera dashboard Streamlit para Loja Importados
# Ajustes feitos: corrigida indentação, removidos blocos duplicados, adicionado sufixo de estoque com emoji
...
def preparar_tabela_vendas(df, estoque_df=None):
    ...
    # try to coerce to numeric where reasonable
    ...
    # preserve numeric raw values for calcula
    try:
...
with tabs[0]:
    ...
    if vendas_filtradas.empty:
        ...
        # add stock suffix in product name (inline) — singular/plural handled, emoji included
        try:
        ...
        # Top 5 — Produtos bombando (with stock inline)
        try:
            ...
            if not vendas_all.empty and \"PRODUTO\" in vendas_all.columns:
                ...
                if not top5.empty:
                    ...
                    def _nome_top5(row):
                        est = int(row.get(\"Estoque\", 0)) if pd.notna(row.get(\"Estoque\", 0)) else 0
                        if est == 1:
                            return f\"{row.get('PRODUTO','')} (📦 Resta {est} produto)\"
                        else:
                            return f\"{row.get('PRODUTO','')} (📦 Resta {est} produtos)\"
...
with tabs[2]:
    ...
    if ordenar == \"Nome A–Z\": df = df.sort_values(\"PRODUTO\", ascending=True)
    elif ordenar == \"Nome Z–A\": df = df.sort_values(\"PRODUTO\", ascending=False)
    elif ordenar == \"Menor preço\": df = df.sort_values(\"Valor Venda Sugerido\", ascending=True)
    elif ordenar == \"Maior preço\": df = df.sort_values(\"Valor Venda Sugerido\", ascending=False)
    elif ordenar == \"Mais vendidos\" and \"TOTAL_QTD\" in df.columns: df = df.sort_values(\"TOTAL_QTD\", ascending=False)
    ...
    with coln1:
        if st.button(\"⬅️ Voltar\"): st.session_state[\"pagina\"] = max(1, st.session_state[\"pagina\"]-1)
    with coln2:
        st.markdown(f\"**Página {st.session_state['pagina']} de {total_paginas} — {total} resultados**\")
    with coln3:
        if st.button(\"Avançar ➡️\"): st.session_state[\"pagina\"] = min(total_paginas, st.session_state[\"pagina\"]+1)
