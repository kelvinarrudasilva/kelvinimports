...
[data-theme="dark"] body, [data-theme="dark"] .stApp { background:#111 !important; color:#f0f0f0 !important; }
[data-theme="dark"] .kpi { background:#1e1e1e; border-left-color:#8b5cf6; color:#f0f0f0; box-shadow:0 6px 16px rgba(255,255,255,0.04); }
[data-theme="dark"] .stDataFrame thead th { background:#222 !important; color:#f0f0f0 !important; }
...
with tabs[2]:
    ...
    if vendas_filtradas.empty: st.info("Sem dados.")
        ...
        fig_top_qtd=px.bar(top_qtd,x="PRODUTO",y="QTD_TOTAL",text="VALOR_TOTAL_LABEL",color_discrete_sequence=["#8b5cf6"])
...
with tabs[3]:
    ...
    if estoque_df.empty: st.info("Sem dados de estoque.")
        ...
        df_estq=estoque_df.copy()
        df_estq["Media C. UNITARIO"]=df_estq.get("Media C. UNITARIO",0)
        df_estq["Valor Venda Sugerido"]=df_estq.get("Valor Venda Sugerido",0)
        df_estq = formatar_colunas_moeda(df_estq, ["Media C. UNITARIO","Valor Venda Sugerido"])
        st.dataframe(df_estq,use_container_width=True)

with tabs[4]:
    st.subheader("Pesquisar Produto")
    termo=st.text_input("Digite o produto")
    if termo:
        resultados=vendas_filtradas[vendas_filtradas["PRODUTO"].str.contains(termo,case=False,na=False)]
        if resultados.empty: st.info("Nenhum resultado encontrado.")
        else: st.dataframe(preparar_tabela_vendas(resultados),use_container_width=True)
