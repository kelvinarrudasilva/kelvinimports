# app.py
# Dashboard NOVE STORE com aba COMPRAS, pesquisa e logo própria
...
def carregar_xlsx_from_url(url: str) -> pd.ExcelFile:
    """
    Baixa a planilha do Google Sheets e retorna um ExcelFile.
    Usamos cache_resource porque ExcelFile não é serializável em cache_data.
    """
...
def limpar_aba_raw(df_raw, nome):
    ...
    df = df.drop(
        columns=[c for c in df.columns if str(c).lower() in ("nan", "none", "")],
        errors="ignore",
    )
...
def compute_encalhados_global(dfs, limit=10):
    ...
    enc = (
        estoque.merge(last_sale, how="left", on="PRODUTO")
        .merge(last_buy, how="left", on="PRODUTO")
    )
...
def preparar_tabela_vendas(df, estoque_df=None):
    ...
    if "DATA" in d.columns:
        try:
            d["_sort"] = pd.to_datetime(
                d["DATA"], format="%d/%m/%Y", errors="coerce"
            )
    ...
    try:
        ...
        d["VALOR VENDA_RAW"] = pd.to_numeric(
            df["VALOR VENDA"], errors="coerce"
        ).fillna(0)
    try:
        d["CUSTO_RAW"] = parse_money_series(df["MEDIA CUSTO UNITARIO"]).fillna(0)
    except Exception:
        d["CUSTO_RAW"] = pd.to_numeric(
            df["MEDIA CUSTO UNITARIO"], errors="coerce"
        ).fillna(0)
    try:
    ...
    try:
        ...
        d["LUCRO TOTAL"] = d["LUCRO TOTAL"].apply(
    ...
    if (
        estoque_df is not None
        and not estoque_df.empty
        ...
        try:
            estoque_for_merge = (
                ...
                .copy()
                .rename(columns={"EM ESTOQUE": "Estoque"})
            )
...
.logo-wrap-img { width:52px; height:52px; border-radius:14px; overflow:hidden; display:flex; align-items:center; justify-content:center; background:#0b0b0b; border:1px solid rgba(255,255,255,0.08); }
...
.avatar{
  ...
  background-size:300% 300%; animation:neonMove 6s ease infinite;
...
# Topbar com logo.png
top_col_logo, top_col_text, top_col_btn = st.columns([0.22, 1.4, 0.4])
with top_col_logo:
    try:
        st.markdown(
        ...
        st.markdown(
            "<div class='logo-wrap-img'><span style='font-weight:800;font-size:22px;'>NS</span></div>",
            unsafe_allow_html=True,
        )
...
with top_col_text:
    st.markdown(
        ...
        <div>
            ...
            <div class="subtitle">Visão de vendas, compras e estoque em tempo real</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
with top_col_btn:
    # Botão de atualizar: limpa cache da planilha e reroda
    if st.button("🔄 Atualizar dados", use_container_width=True):
        try:
            carregar_xlsx_from_url.clear()  # limpa cache_resource
        except Exception:
            pass
...
try:
    ...
    st.error("Erro ao abrir a planilha. Verifique o link do Google Sheets.")
    st.exception(e)
    st.stop()
...
if "ESTOQUE" in dfs:
    ...
    for alt in ["EM ESTOQUE", "ESTOQUE", "QTD", "QUANTIDADE"]:
        if alt in df_e.columns:
            df_e["EM ESTOQUE"] = (
                parse_int_series(df_e[alt]).fillna(0).astype(int)
            )
            break
...
if "VENDAS" in dfs:
    ...
    if qtd_cols:
        df_v["QTD"] = (
            parse_int_series(df_v[qtd_cols[0]]).fillna(0).astype(int)
        )
    ...
    if "VALOR TOTAL" not in df_v and "VALOR VENDA" in df_v:
        df_v["VALOR TOTAL"] = (
            df_v["VALOR VENDA"].fillna(0)
            * df_v.get("QTD", 0).fillna(0)
        )
    if "LUCRO UNITARIO" not in df_v and (
        "VALOR VENDA" in df_v and "MEDIA CUSTO UNITARIO" in df_v
    ):
        df_v["LUCRO UNITARIO"] = (
            df_v["VALOR VENDA"].fillna(0)
            - df_v["MEDIA CUSTO UNITARIO"].fillna(0)
        )
    if "DATA" in df_v.columns:
...
if "COMPRAS" in dfs:
    ...
    if qcols:
        df_c["QUANTIDADE"] = (
            parse_int_series(df_c[qcols[0]]).fillna(0).astype(int)
        )
    ccols = [
        c
        for c in df_c.columns
        if any(k in c.upper() for k in ("CUSTO", "UNIT", "VALOR"))
    ]
    if ccols:
        df_c["CUSTO UNITÁRIO"] = parse_money_series(df_c[ccols[0]]).fillna(0)
    df_c["CUSTO TOTAL (RECALC)"] = df_c.get("QUANTIDADE", 0) * df_c.get(
        "CUSTO UNITÁRIO", 0
    )
...
if not estoque_df.empty:
    estoque_df["Media C. UNITARIO"] = (
        estoque_df.get("Media C. UNITARIO", 0).fillna(0).astype(float)
    )
    estoque_df["Valor Venda Sugerido"] = (
        estoque_df.get("Valor Venda Sugerido", 0).fillna(0).astype(float)
    )
    estoque_df["EM ESTOQUE"] = (
        estoque_df.get("EM ESTOQUE", 0).fillna(0).astype(int)
    )
    valor_custo_estoque = (
        estoque_df["Media C. UNITARIO"] * estoque_df["EM ESTOQUE"]
    ).sum()
    valor_venda_estoque = (
        estoque_df["Valor Venda Sugerido"] * estoque_df["EM ESTOQUE"]
    ).sum()
...
if "VENDAS" in dfs:
    meses += sorted(
        dfs["VENDAS"]["MES_ANO"].dropna().unique().tolist(), reverse=True
    )
...
with col_filter:
    mes_selecionado = st.selectbox(
        "Filtrar por mês (YYYY-MM):", meses, index=index_padrao
    )
...
if not vendas_filtradas.empty and "DATA" in vendas_filtradas.columns:
    vendas_filtradas = vendas_filtradas.sort_values(
        "DATA", ascending=False
    ).reset_index(drop=True)
compras_filtradas = filtrar_mes_df(
    dfs.get("COMPRAS", pd.DataFrame()), mes_selecionado
)

# KPIs
total_vendido = (
    vendas_filtradas.get("VALOR TOTAL", pd.Series()).fillna(0).sum()
)
total_lucro = (
    vendas_filtradas.get("LUCRO UNITARIO", 0).fillna(0)
    * vendas_filtradas.get("QTD", 0).fillna(0)
).sum()
total_compras = (
    compras_filtradas.get("CUSTO TOTAL (RECALC)", pd.Series())
    .fillna(0)
    .sum()
)
...
with col_kpis:
    ...
    mostrar_saldos = st.toggle(
        "👁‍🗨 Mostrar faturamento e lucro", value=False, help="Por segurança, os valores ficam ocultos por padrão."
    )
...
with tabs[0]:
    ...
    if vendas_filtradas.empty:
        ...
        df_sem = df_sem.sort_values("DATA", ascending=False).reset_index(
            drop=True
        )
        ...
        def semana_intervalo(row):
            try:
                inicio = datetime.fromisocalendar(
                    int(row["ANO"]), int(row["SEMANA"]), 1
                )
        ...
        df_sem_group = (
            df_sem.groupby(["ANO", "SEMANA"], dropna=False)["VALOR TOTAL"]
            .sum()
            .reset_index()
        )
        if not df_sem_group.empty:
            df_sem_group["INTERVALO"] = df_sem_group.apply(
                semana_intervalo, axis=1
            )
            df_sem_group["LABEL"] = df_sem_group["VALOR TOTAL"].apply(
                formatar_reais_com_centavos
            )
        ...
        tabela_vendas_exib = preparar_tabela_vendas(
            df_sem, estoque_df=estoque_df
        )

        try:
            if (
                not tabela_vendas_exib.empty
                ...
                tabela_vendas_exib["PRODUTO"] = tabela_vendas_exib.apply(
                    _nome_prod, axis=1
                )
        except Exception:
            pass

        cols_v = tabela_vendas_exib.columns.tolist()
        if "OBS" in cols_v:
            limite = cols_v.index("OBS") + 1
            tabela_vendas_exib = tabela_vendas_exib[cols_v[:limite]]
        ...
        try:
            ...
            if not vendas_all.empty and "PRODUTO" in vendas_all.columns:
                top5 = (
                    vendas_all.groupby("PRODUTO")["QTD"]
                    .sum()
                    .reset_index()
                ...
                if not top5.empty:
                    ...
                    if not est.empty and "PRODUTO" in est.columns:
                        top5 = top5.merge(
                            est[["PRODUTO", "EM ESTOQUE"]].rename(
                                columns={"EM ESTOQUE": "Estoque"}
                            ),
                            on="PRODUTO",
                            how="left",
                        )
                    ...
                    top5_display["Produto"] = top5_display.apply(
                        _nome_top5, axis=1
                    )
                    top5_display = top5_display.rename(
                        columns={"QTD": "Unidades"}
                    )
                    st.markdown(
                        "### 🔥 Top 5 — Produtos bombando (por unidades vendidas)"
                    )
        ...
        try:
            ...
            if not compras_all.empty:
                compras_all["DATA"] = pd.to_datetime(
                    compras_all["DATA"], errors="coerce"
                )
            if not vendas_all.empty:
                vendas_all["DATA"] = pd.to_datetime(
                    vendas_all["DATA"], errors="coerce"
                )
            if not estoque_all.empty:
                if not vendas_all.empty:
                    ...
                    last_sale = pd.DataFrame(
                        columns=["PRODUTO", "ULT_VENDA"]
                    )
                if not compras_all.empty:
                    last_buy = (
                        compras_all.groupby("PRODUTO")["DATA"]
                        .max()
                        .reset_index()
                        .rename(columns={"DATA": "ULT_COMPRA"})
                    )
                else:
                    last_buy = pd.DataFrame(
                        columns=["PRODUTO", "ULT_COMPRA"]
                    )
                enc = (
                    estoque_all.merge(
                        last_sale, how="left", on="PRODUTO"
                    ).merge(last_buy, how="left", on="PRODUTO")
                )
                ...
                if not enc.empty:
                    ...
                    enc_display["ULT_VENDA"] = enc_display[
                        "ULT_VENDA"
                    ].dt.strftime("%d/%m/%Y").fillna("—")
                    enc_display["ULT_COMPRA"] = enc_display[
                        "ULT_COMPRA"
                    ].dt.strftime("%d/%m/%Y").fillna("—")
                    st.markdown(
                        "### ❄️ Produtos encalhados (global) — última venda/compra e estoque atual"
                    )
                    st.table(
                        enc_display.rename(
                            columns={
...
with tabs[1]:
    if estoque_df.empty:
        ...
        estoque_display["VALOR_CUSTO_TOTAL_RAW"] = (
            estoque_display["Media C. UNITARIO"]
            * estoque_display["EM ESTOQUE"]
        ).fillna(0)
        estoque_display["VALOR_VENDA_TOTAL_RAW"] = (
            estoque_display["Valor Venda Sugerido"]
            * estoque_display["EM ESTOQUE"]
        ).fillna(0)

        st.markdown("### 🥧 Distribuição de estoque — top itens por quantidade")
        top_for_pie = estoque_display.sort_values(
            "EM ESTOQUE", ascending=False
        ).head(10)
        if not top_for_pie.empty:
            ...
            fig_pie.update_traces(
                ...
                pull=[
                    0.05 if i == 0 else 0
                    for i in range(len(top_for_pie))
                ],
                marker=dict(
            ...
            fig_pie.update_layout(
                title={
                    "text": "Top itens por quantidade em estoque",
                    "y": 0.96,
                    "x": 0.5,
                    "xanchor": "center",
                },
        ...
        estoque_clas["CUSTO_UNITARIO_FMT"] = estoque_clas[
            "Media C. UNITARIO"
        ].map(formatar_reais_com_centavos)
        estoque_clas["VENDA_SUGERIDA_FMT"] = estoque_clas[
            "Valor Venda Sugerido"
        ].map(formatar_reais_com_centavos)
        estoque_clas["VALOR_TOTAL_CUSTO_FMT"] = estoque_clas[
            "VALOR_CUSTO_TOTAL_RAW"
        ].map(formatar_reais_sem_centavos)
        estoque_clas["VALOR_TOTAL_VENDA_FMT"] = estoque_clas[
            "VALOR_VENDA_TOTAL_RAW"
        ].map(formatar_reais_sem_centavos)
...
with tabs[2]:
    ...
    if df_c.empty:
        ...
        if "QUANTIDADE" in df_c.columns:
            df_c["QUANTIDADE"] = (
                parse_int_series(df_c["QUANTIDADE"])
                .fillna(0)
                .astype(int)
            )
        if "CUSTO UNITÁRIO" in df_c.columns:
            df_c["CUSTO UNITÁRIO"] = parse_money_series(
                df_c["CUSTO UNITÁRIO"]
            ).fillna(0)
        if "CUSTO TOTAL (RECALC)" not in df_c.columns:
            df_c["CUSTO TOTAL (RECALC)"] = df_c.get(
                "QUANTIDADE", 0
            ) * df_c.get("CUSTO UNITÁRIO", 0)

        compras_mes = filtrar_mes_df(df_c, mes_selecionado)
        compras_mes = compras_mes.sort_values(
            "DATA", ascending=False
        ).reset_index(drop=True)
        ...
        def is_marketing_row(row):
            ...
            for c in df_c.columns:
                if any(
                    k in str(c).upper()
                    for k in ("DESCR", "OBS", "NOTA", "FINAL", "MOTIVO")
                ) and pd.notna(row.get(c, "")):
                    text += " " + str(row.get(c, ""))
            text = text.upper()
            return any(kw in text for kw in marketing_keywords)

        compras_mes["_MARKETING"] = compras_mes.apply(
            is_marketing_row, axis=1
        )

        marketing_total = compras_mes.loc[
            compras_mes["_MARKETING"], "CUSTO TOTAL (RECALC)"
        ].fillna(0).sum()
        marketing_count = compras_mes["_MARKETING"].sum()
        marketing_pct = (
            marketing_total / total_comp_mes * 100 if total_comp_mes else 0
        )

        st.markdown("### 📈 Visão rápida das compras")

        group_by_for = fornecedor_col if fornecedor_col is not None else "PRODUTO"
        group_col = (
            group_by_for if group_by_for in compras_mes.columns else "PRODUTO"
        )
        pivot = (
            compras_mes.groupby(group_col)["CUSTO TOTAL (RECALC)"]
            .sum()
            .reset_index()
            .sort_values("CUSTO TOTAL (RECALC)", ascending=False)
        )
        pivot["CUSTO_FMT"] = pivot["CUSTO TOTAL (RECALC)"].map(
        ...
        for c in df_c.columns:
            if any(
                k in str(c).upper()
                for k in ("CATEG", "FINAL", "TIPO", "DEST", "USO")
            ):
                category_col = c
                break
        treemap_col = (
            category_col if category_col in compras_mes.columns else "PRODUTO"
        )
        try:
            tm = (
                compras_mes.groupby(treemap_col)["CUSTO TOTAL (RECALC)"]
                .sum()
                .reset_index()
                .sort_values("CUSTO TOTAL (RECALC)", ascending=False)
            )
        ...
        if marketing_total > 0:
            ...
            insights.append(
                "Nenhuma compra claramente marcada como 'marketing' encontrada — vale padronizar a descrição para identificar gastos de anúncio/divulgação."
            )
        if not pivot.empty:
            top_conc = pivot["CUSTO TOTAL (RECALC)"].iloc[0]
            if total_comp_mes > 0 and top_conc / total_comp_mes > 0.4:
                insights.append(
                    "Alerta: mais de 40% dos custos concentrados em 1 fornecedor/produto — atenção à dependência."
                )
        insights.append(
            "Sugestão: use uma palavra-chave padrão (ex: 'ADS META' ou 'MARKETING') em OBS para facilitar a análise de retorno."
        )
        for ins in insights:
            st.write("- " + ins)
...
with tabs[3]:
    ...
    with col_b:
        ...
        with cols_sel[0]:
            itens_pagina = st.selectbox(
                "Itens/pg", [6, 9, 12, 24, 36, 48, 60, 100, 200], index=2
            )
        with cols_sel[1]:
    ...
    if not vendas_df.empty and "QTD" in vendas_df.columns:
        vend = (
            vendas_df.groupby("PRODUTO")["QTD"]
            .sum()
            .reset_index()
            .rename(columns={"QTD": "TOTAL_QTD"})
        )
        df = df.merge(vend, how="left", on="PRODUTO").fillna(
            {"TOTAL_QTD": 0}
        )
    else:
        df["TOTAL_QTD"] = 0
    ...
    df["CUSTO_FMT"] = df.get("Media C. UNITARIO", 0).map(
        formatar_reais_com_centavos
    )
    df["VENDA_FMT"] = df.get("Valor Venda Sugerido", 0).map(
        formatar_reais_com_centavos
    )
    ...
    st.session_state["pagina"] = max(
        1, min(st.session_state["pagina"], total_paginas)
    )
    coln1, coln2, coln3 = st.columns([1, 2, 1])
    with coln1:
        if st.button("⬅️ Voltar"):
            st.session_state["pagina"] = max(
                1, st.session_state["pagina"] - 1
            )
    with coln2:
        st.markdown(
            f"**Página {st.session_state['pagina']} de {total_paginas} — {total} resultados**"
        )
    with coln3:
        if st.button("Avançar ➡️"):
            st.session_state["pagina"] = min(
                total_paginas, st.session_state["pagina"] + 1
            )
    ...
    for _, r in df_page.iterrows():
        nome = r.get("PRODUTO", "")
        estoque_val = r.get("EM ESTOQUE", 0)
        estoque = int(estoque_val) if pd.notna(estoque_val) else 0
        venda = r.get("VENDA_FMT", "R$ 0")
        custo = r.get("CUSTO_FMT", "R$ 0")
        vendidos_val = r.get("TOTAL_QTD", 0)
        vendidos = int(vendidos_val) if pd.notna(vendidos_val) else 0
        iniciais = (
            "".join(
                [p[0].upper() for p in str(nome).split()[:2] if p]
            )
            or "—"
        )
        badges = []
        ...
        if nome in ultima_compra and vendidos == 0:
            vendas_produto = (
                vendas_df[vendas_df["PRODUTO"] == nome]
                if not vendas_df.empty
                else pd.DataFrame()
            )
            if vendas_produto.empty:
                badges.append(
                    "<span class='badge zero'>❄️ Sem vendas</span>"
                )
        try:
...
<div style='margin-top:6px; line-height:1.25;'>
  <div style='font-size:13px; color:#ffffff; font-weight:700;'>
    💲 Venda: <span style='color:#a78bfa;'>{venda}</span>
  </div>
  <div style='font-size:12px; color:#cfcfe0;'>
    💰 Custo: <span style='color:#ffb4b4;'>{custo}</span>
  </div>
</div>
"""
