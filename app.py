...
st.markdown("""
<style>
body, .stApp { background:#fff; color:#111; font-family: Inter, system-ui; }
.topbar { display:flex; align-items:center; gap:12px; margin-bottom:8px; }
.logo-wrap { width:44px; height:44px; display:flex; align-items:center; justify-content:center; border-radius:10px; background: linear-gradient(135deg,#8b5cf6,#6d28d9); box-shadow:0 6px 18px rgba(109,40,217,0.12); }
.logo-wrap svg { width:26px; height:26px; }
.title { font-size:20px; font-weight:800; color:#6d28d9; margin:0; line-height:1; }
.subtitle { margin:0; font-size:12px; color:#666; margin-top:2px; }
.kpi-row { display:flex; gap:10px; align-items:center; margin-bottom:20px; }
.kpi { background:#fff; border-radius:10px; padding:10px 14px; box-shadow:0 6px 16px rgba(13,12,20,0.04); border-left:6px solid #8b5cf6; min-width:160px; display:flex; flex-direction:column; justify-content:center; }
.kpi h3 { margin:0; font-size:12px; color:#6d28d9; font-weight:800; letter-spacing:0.2px; }
.kpi .value { margin-top:6px; font-size:20px; font-weight:900; color:#111; white-space:nowrap; }
.stTabs { margin-top:20px !important; }
.stTabs button { background:white !important; border:1px solid #f0eaff !important; border-radius:12px !important; padding:8px 14px !important; margin-right:8px !important; margin-bottom:8px !important; font-weight:700 !important; color:#6d28d9 !important; box-shadow:0 3px 10px rgba(0,0,0,0.04) !important; }
...
def preparar_tabela_vendas(df):
    ...
    # remover MES_ANO e índice padrão
    d = d.loc[:, [col for col in d.columns if col!="MES_ANO"]]
...
if "ESTOQUE" in dfs:
    ...
    for c in ["Media C. UNITARIO","Valor Venda Sugerido"]: 
        if c in df_e.columns: df_e[c] = parse_money_series(df_e[c])
    for c in ["EM ESTOQUE","VENDAS"]: 
        if c in df_e.columns: df_e[c] = parse_int_series(df_e[c]).fillna(0)
    dfs["ESTOQUE"]=df_e

if "VENDAS" in dfs:
    df_v = dfs["VENDAS"]
    for col in df_v.columns:
        if any(k in col.upper() for k in ["VALOR","TOTAL"]): df_v[col]=parse_money_series(df_v[col])
    if "QTD" in df_v.columns: df_v["QTD"]=parse_int_series(df_v["QTD"]).fillna(0)
    if "DATA" in df_v.columns: df_v["DATA"]=pd.to_datetime(df_v["DATA"], errors="coerce"); df_v["MES_ANO"]=df_v["DATA"].dt.strftime("%Y-%m")
    dfs["VENDAS"]=df_v

if "COMPRAS" in dfs:
    df_c = dfs["COMPRAS"]
    if any("QUANT" in c.upper() for c in df_c.columns): df_c["QUANTIDADE"]=parse_int_series(df_c[[c for c in df_c.columns if "QUANT" in c.upper()][0]]).fillna(0)
    if any("CUSTO" in c.upper() or "UNIT" in c.upper() for c in df_c.columns): df_c["CUSTO UNITÁRIO"]=parse_money_series(df_c[[c for c in df_c.columns if "CUSTO" in c.upper() or "UNIT" in c.upper()][0]]).fillna(0)
...
# Filtrar mês
# =============================
meses = sorted(dfs.get("VENDAS",pd.DataFrame()).get("MES_ANO",pd.Series()).dropna().unique().tolist(), reverse=True)
...
def filtrar_mes_df(df, mes):
    ...
    return df[df.get("MES_ANO",None)==mes].copy()
...
with tabs[0]:
    ...
    if vendas_filtradas.empty: st.info("Sem dados de vendas.")
    else:
        ...
        def intervalo(row):
            inicio=datetime.fromisocalendar(row["ANO"], row["SEMANA"], 1)
        ...
        df_sem_group["INTERVALO"]=df_sem_group.apply(intervalo,axis=1)
...
with tabs[1]:
    ...
    if vendas_filtradas.empty: st.info("Sem dados.")
        ...
        disp=top_val.copy(); disp["VALOR TOTAL"]=disp["VALOR_TOTAL"].map(formatar_reais_sem_centavos)
...
with tabs[3]:
    ...
    if estoque_df.empty: st.info("Aba ESTOQUE não encontrada.")
    else:
        d=estoque_df.copy()
